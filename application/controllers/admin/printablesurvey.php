    ...
    function index($surveyid, $lang = null, $bReturn = false)
    {
        ...
        if (!Permission::model()->hasSurveyPermission($surveyid, 'surveycontent', 'read')) {
            ...
            if (isset($showsgqacode) && $showsgqacode == true) {
                $surveyname = "<h3 style='text-align:center;'>".$surveyname."</h3><br />[".gT('Database')." ".gT('table').": $surveytable]";
				//Hager
            }
            ...
            $printarray = array(
                ...
                'submit_text' => "<input type='submit' />".gT("Submit Your Survey."),
                'end' => $end,
...
// TIBO
                        ...
                        switch ($deqrow['type']) {
                            ...
                            case "5":    //5 POINT CHOICE
                                ...
                                for ($i = 1; $i <= 5; $i++) {
                                    //$question['answer'] .= "\t\t<li>\n\t\t\t".self::_input_type_image('radio', $i)."\t\t\t$i ".self::_addsgqacode("($i)")."\n\t\t</li>\n";
                                    $question['answer'] .= "\t\t<li>\n\t\t\t".'<input type="radio" value="'.$i.'"/>'."\t\t\t$i ".self::_addsgqacode("($i)")."\n\t\t</li>\n";
                                }
                                $question['answer'] .= "\t</ul>\n";
                                ...
                                // ==================================================================
                            case "D":  //DATE
                                $question['type_help'] .= CHtml::tag("div", array("class"=>"tip-help"), gT('Please enter a date:format: mm.dd.yyyy'));
                            ...
                            case "G":  //GENDER
                                ...
                                $question['answer'] .= "\n\t<ul class='list-print-answers list-unstyled'>\n";
                                //$question['answer'] .= "\t\t<li>\n\t\t\t".self::_input_type_image('radio', gT("Female"))."\t\t\t".gT("Female")." ".self::_addsgqacode("(F)")."\n\t\t</li>\n";
                                $question['answer'] .= "\t\t<li>\n\t\t\t".'<input type="radio" value="F"/>'.gT("Female")." \n\t\t</li>\n";
                                $question['answer'] .= "\t\t<li>\n\t\t\t".'<input type="radio" value="M"/>'.gT("Male")." \n\t\t</li>\n";
                                //$question['answer'] .= "\t\t<li>\n\t\t\t".self::_input_type_image('radio', gT("Male"))."\t\t\t".gT("Male")." ".self::_addsgqacode("(M)")."\n\t\t</li>\n";
                                //$question['answer'] .= "\t\t<li>\n\t\t\t".self::_input_type_image('radio', gT("Male"))."\t\t\t".gT("Male")." ".self::_addsgqacode("(M)")."\n\t\t</li>\n";
                                //$output = '<input type="radio" value="M"/><input type="radio" value="F"/>';
								$question['answer'] .= "\t</ul>\n";
                            ...
                            case "!": //List - dropdown
                                ...
                                foreach ($dearesult as $dearow) {
                                    if (isset($optCategorySeparator)) {
                                        ...
                                        //$question['answer'] .= "\t".$wrapper['item-start']."\t\t".self::_input_type_image('radio', $dearow['answer'])."\t\t\t".$dearow['answer']."\n".$wrapper['item-end'];
                                        //17:24
										$question['answer'] .= "\t".$wrapper['item-start']."\t\t".'<input type="radio" value="A'.$i.'"/>'."\t\t\t".$dearow['answer']."\n".$wrapper['item-end'];
                                    } else {
                                        //$question['answer'] .= "\t".$wrapper['item-start']."\t\t".self::_input_type_image('radio', $dearow['answer'])."\t\t\t".$dearow['answer'].self::_addsgqacode(" (".$dearow['code'].")")."\n".$wrapper['item-end'];
										$question['answer'] .= "\t".$wrapper['item-start']."\t\t".'<input type="radio" value="A'.$i.'"/>'."\t\t\t".$dearow['answer']."\n".$wrapper['item-end'];
                                    }
                                    ++$rowcounter;
                            ...
                            case "R":  //RANKING Type Question
                                ...
                                foreach ($rearesult as $rearow) {
                                    ...
                                    $question['answer'] .= "\t".'<input type="radio" value="A'.$i.'"/>'."\n";
                            ...
                            case "M":  //Multiple choice (Quite tricky really!)
                                ...
                                foreach ($mearesult as $mearow) {
                                    ...
                                    $question['answer'] .= $wrapper['item-start'].'<input type="checkbox" value="Y"/>'."\n\t\t".$mearow['question'].self::_addsgqacode(" (".$fieldname.$mearow['title'].") ").$wrapper['item-end'];
                                    ++$rowcounter;
                                    if ($rowcounter == $wrapper['maxrows'] && $colcounter < $wrapper['cols']) {
                                        if ($colcounter == $wrapper['cols'] - 1) {
                                            $question['answer'] .= $wrapper['col-devide-last'];
                                ...
                                if ($deqrow['other'] == "Y") {
                                    ...
                                    $question['answer'] .= $wrapper['item-start-other'].'<input type="checkbox" value="Y"/>'.gT($qidattributes["other_replace_text"][$sLanguageCode]).":\n\t\t".self::_input_type_image('other').self::_addsgqacode(" (".$fieldname."other) ").$wrapper['item-end'];
                                }
                                $question['answer'] .= $wrapper['whole-end'];
                                //                }
                                break;
                            ...
                            case "P":  //Multiple choice with comments
                                ...
                                foreach ($mearesult->readAll() as $mearow) {
                                    ...
                                    $question['answer'] .= "<div class='col-sm-{$aWidth['label']}'>\n\t\t".'<input type="checkbox" value="Y"/>'.$mearow['question'].self::_addsgqacode(" (".$fieldname.$mearow['title'].") ")."</div>\n";
                            ...
                            case "Y":  //YES/NO
                                  $question['type_help'] .= CHtml::tag("div", array("class"=>"tip-help"), gT("Please choose *only one* of the following:"));
                                //$question['answer'] = "\n<ul class='list-print-answers list-unstyled'>\n\t<li>\n\t\t".self::_input_type_image('radio', gT('Yes'))."\t\t".gT('Yes').self::_addsgqacode(" (Y)")."\n\t</li>\n";
                                $question['answer'] = "\n<ul class='list-print-answers list-unstyled'>\n\t<li>\n\t\t";
                                //$question['answer'] .= "\n\t<li>\n\t\t".self::_input_type_image('radio', gT('No'))."\n\t\t".gT('No').self::_addsgqacode(" (N)")."\t</li>\n</ul>\n";
                            ...
                            case "A":  //ARRAY (5 POINT CHOICE)
                                ...
                                foreach ($mearesult->readAll() as $mearow) {
                                    ...
                                    for ($i = 1; $i <= 5; $i++) {
                                        ...
                                        $question['answer'] .= "\t\t\t<td>".'<input type="radio" value="'.$i.'" >'."</td>\n";
                                    }
                            ...
                            case "C":  //ARRAY (YES/UNCERTAIN/NO)
                                ...
                                foreach ($mearesult->readAll() as $mearow) {
                                    ...
                                    $question['answer'] .= "\t\t\t".'<td><input type="radio" value="Y" />'."</td>\n";
                                    //$question['answer'] .= "\t\t\t<td>".self::_input_type_image('radio', gT("Uncertain"))."</td>\n";
                                    $question['answer'] .= "\t\t\t".'<td><input type="radio" value="U" />'."</td>\n";
                                    //$question['answer'] .= "\t\t\t<td>".self::_input_type_image('radio', gT("No"))."</td>\n";
                                    $question['answer'] .= "\t\t\t".'<td><input type="radio" value="N" />'."</td>\n";
                                    $question['answer'] .= "\t\t</tr>\n";
                            ...
                            case "E":  //ARRAY (Increase/Same/Decrease)
                                ...
                                foreach ($mearesult->readAll() as $mearow) {
                                    ...
                                    $question['answer'] .= "\t\t\t<th class=\"answertext\">{$mearow['question']}".self::_addsgqacode(" (".$fieldname.$mearow['title'].")")."</th>\n";
                                    $question['answer'] .= "\t\t\t<td>".'<input type="radio" value="I"/>'."</td>\n";
                                    $question['answer'] .= "\t\t\t<td>".'<input type="radio" value="S"/>'."</td>\n";
                                    $question['answer'] .= "\t\t\t<td>".'<input type="radio" value="D"/>'."</td>\n";
                                    $question['answer'] .= "\t\t</tr>\n";
                                    $j++;
                                    $rowclass = alternation($rowclass, 'row');
                                }
                                ...
                                foreach ($result as $frow) {
                                    ...
                                    for ($i = 1; $i <= $fcount; $i++) {
                                        ...
                                        if ($checkboxlayout === false) {
                                            ...
                                            //$question['answer'] .= "\t\t\t\t".'<input type="checkbox" class="array" value="'.self::_addsgqacode(" (".$fieldname.$frow['title']."_".$xaxisarray[$i].") ").'">'.self::_addsgqacode(" (".$fieldname.$frow['title']."_".$xaxisarray[$i].") ")."\n";
                                            $question['answer'] .= "\t\t\t\t".'<input type="checkbox" class="array" value="Y">'.self::_addsgqacode(" (".$fieldname.$frow['title']."_".$xaxisarray[$i].") ")."\n";
                                        }
                                        $question['answer'] .= "\t\t\t</td>\n";
                                    }
                                    $answertext = $frow['question'];
                            ...
                            case "F": //ARRAY (Flexible Labels)
                                ...
                                foreach ($mearesult->readAll() as $mearow) {
                                    ...
                                    for ($i = 1; $i <= $fcount; $i++) {
                                        ...
                                        $question['answer'] .= "\t\t\t<td>".'<input type="radio" value="A'.$i.'">'."</td>\n";
    ...
    private function _input_type_image($type, $title = null, $size = null, $rows = null)
    {
        ...
        switch ($type) {
            ...
            case 'text':
            case 'textarea':
                $output = '<div contentEditable="true" style="color: #636363;background-color: #fff;border: 1px solid #ccc;box-shadow: inset 0 1px 1px rgba(0,0,0,0.075);" class="form-control print-control input-'.$type.'"'.$style.$div_title.'><span></span></div>';
                //Hager
				break;
            default:
                $output = '';
